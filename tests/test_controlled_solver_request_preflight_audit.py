...
def test_request_preflight_no_execution_audit_passes(tmp_path: Path) -> None:
    ...
    audit = audit_controlled_solver_request_preflight_no_execution(task, result)
    assert audit["audit_status"] == "CONTROLLED_SOLVER_REQUEST_PREFLIGHT_NO_EXECUTION_AUDIT_PASS"
