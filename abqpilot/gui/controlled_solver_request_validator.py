...
READY = "CONTROLLED_SOLVER_REQUEST_DRAFT_SCHEMA_READY"
MISSING_SOURCE = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_MISSING_SOURCE_GATE_OR_HANDOFF"
SOURCE_GATE_INVALID = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_SOURCE_GATE_INVALID"
HANDOFF_INVALID = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_HANDOFF_INVALID"
HASH_MISMATCH = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_CANDIDATE_HASH_MISMATCH"
EXECUTION_ALLOWED = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_EXECUTION_ALLOWED"
ACTIVE_REQUEST = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_ACTIVE_REQUEST"
REQUEST_CREATED = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_SOLVER_REQUEST_CREATED"
QUEUE_OR_ODB = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_QUEUE_OR_ODB"
FINAL_EVIDENCE = "CONTROLLED_SOLVER_REQUEST_DRAFT_BLOCKED_FINAL_EVIDENCE"
REVIEW_REQUIRED = "CONTROLLED_SOLVER_REQUEST_DRAFT_REVIEW_REQUIRED"


def validate_controlled_solver_request_draft(draft: dict[str, Any]) -> dict[str, Any]:
    required = {
    ...
    if not checks["draft_type_ok"] or not checks["stage_id_ok"]:
        ...
        status = SOURCE_GATE_INVALID
    elif not checks["source_handoff_draft_validated_ok"]:
        status = HANDOFF_INVALID
    elif not checks["candidate_hash_verified_ok"]:
        status = HASH_MISMATCH
    elif draft.get("request_active") is True or draft.get("executable_request") is True:
        ...
        or draft.get("solver_command_not_invoked") is False
        or draft.get("output_dir_created") is True
    ):
    ...
    return {
        ...
        "request_active": draft.get("request_active") is True,
        "executable_request": draft.get("executable_request") is True,
