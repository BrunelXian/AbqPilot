...
def test_gui_app_imports_for_stage5_2i() -> None:
    assert main is not None


def test_preflight_card_is_callback_free() -> None:
    card = build_controlled_solver_request_preflight_card()
    assert card["title"] == "Controlled Solver Request Preflight [NO EXECUTION]"
    assert card["preflight_only"] is True
    ...
    assert "Preflight only; no solver execution" in card["required_copy"]
...
def test_preflight_panel_action_has_no_execute_callback() -> None:
    panel = build_controlled_solver_gate_panel()
    action = [item for item in panel["actions"] if item["action_id"] == "report_controlled_solver_stage5_2i_request_preflight"][0]
