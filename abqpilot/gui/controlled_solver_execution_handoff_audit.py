...
PASS = "CONTROLLED_SOLVER_HANDOFF_DRAFT_NO_EXECUTION_AUDIT_PASS"
FAIL = "CONTROLLED_SOLVER_HANDOFF_DRAFT_NO_EXECUTION_AUDIT_FAIL"
...
def audit_controlled_solver_execution_handoff_no_execution(task_dir: str | Path, draft: dict[str, Any] | None = None) -> dict[str, Any]:
    task = Path(task_dir)
    solver_request_files = [str(path) for path in task.rglob("*") if path.is_file() and path.name in SOLVER_REQUEST_FILENAMES] if task.exists() else []
    ...
    draft = draft or {}
    checks = {
        "no_solver_request_files_found": not solver_request_files,
        "no_job_request_files_found": not any(Path(path).name == "job_request.json" for path in solver_request_files),
        "no_abaqus_job_files_found": not any(Path(path).name == "abaqus_job.json" for path in solver_request_files),
        ...
        "draft_solver_run_false": draft.get("solver_run") is not True,
        "draft_solver_execution_allowed_false": draft.get("solver_execution_allowed") is not True,
        "draft_downstream_execution_auto_start_false": draft.get("downstream_execution_auto_start") is not True,
    }
    return {
        "schema_version": "0.1",
        "stage": "Stage 5.2G",
        ...
        "checks": checks,
        "solver_request_files": solver_request_files,
