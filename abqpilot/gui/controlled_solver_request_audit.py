...
PASS = "CONTROLLED_SOLVER_REQUEST_DRAFT_NO_EXECUTION_AUDIT_PASS"
FAIL = "CONTROLLED_SOLVER_REQUEST_DRAFT_NO_EXECUTION_AUDIT_FAIL"

ACTIVE_REQUEST_NAMES = {"solver_request.json", "job_request.json", "abaqus_job.json"}


def audit_controlled_solver_request_draft_no_execution(task_dir: str | Path, draft: dict[str, Any] | None = None) -> dict[str, Any]:
    task = Path(task_dir)
    ...
    checks = {
        ...
        "draft_request_active_false": draft.get("request_active") is not True,
        "draft_executable_request_false": draft.get("executable_request") is not True,
