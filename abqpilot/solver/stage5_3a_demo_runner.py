...
JOB_NAME = "stage5_3a_demo_solver_smoke"
CPU_COUNT = 4
TIMEOUT_SECONDS = 1800


def build_stage5_3a_demo_command(candidate_inp: str | Path) -> list[str]:
    inp = Path(candidate_inp)
    return [
        ABAQUS_COMMAND,
        f"job={JOB_NAME}",
        f"input={inp}",
...
def execute_stage5_3a_demo_solver_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    command = build_stage5_3a_demo_command(inp)
    if str(command_path) != ABAQUS_COMMAND or command[0] != ABAQUS_COMMAND:
        return _blocked("STAGE5_3A_BLOCKED_UNSAFE_COMMAND_SHAPE", command, work_dir, timeout)
    if request.get("task_id") != "stage5_3a_controlled_solver_demo_smoke":
        return _blocked("STAGE5_3A_BLOCKED_ARBITRARY_TASK_PATH", command, work_dir, timeout)
    ...
    if request.get("solver_execution_allowed") is not True or request.get("executable_request") is not True:
        return _blocked("STAGE5_3A_BLOCKED_REQUEST_NOT_EXECUTABLE_FOR_DEMO", command, work_dir, timeout)
    if not command_path.exists():
        return _blocked("STAGE5_3A_BLOCKED_ABAQUS_COMMAND_NOT_FOUND", command, work_dir, timeout)
    if not inp.exists():
        return _blocked("STAGE5_3A_BLOCKED_DEMO_INP_NOT_FOUND", command, work_dir, timeout)
    ...
    try:
        ...
        report["solver_status"] = (
            "STAGE5_3A_SOLVER_RUN_COMPLETED_RETURN_ZERO"
            if completed.returncode == 0
            else "STAGE5_3A_SOLVER_RUN_COMPLETED_NONZERO_RETURN"
        )
    except subprocess.TimeoutExpired as exc:
        report["timed_out"] = True
        report["stdout_tail"] = _tail(exc.stdout or "")
        report["stderr_tail"] = _tail(exc.stderr or "")
        report["solver_status"] = "STAGE5_3A_SOLVER_RUN_TIMED_OUT"
    except OSError as exc:
        report["solver_invoked"] = False
        report["errors"].append(str(exc))
        report["solver_status"] = "STAGE5_3A_SOLVER_RUN_OS_ERROR"
    return report
