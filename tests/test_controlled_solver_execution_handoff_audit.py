...
def test_execution_handoff_no_execution_audit_passes(tmp_path: Path) -> None:
    ...
    audit = audit_controlled_solver_execution_handoff_no_execution(task, result["draft"])
    assert audit["audit_status"] == "CONTROLLED_SOLVER_HANDOFF_DRAFT_NO_EXECUTION_AUDIT_PASS"
